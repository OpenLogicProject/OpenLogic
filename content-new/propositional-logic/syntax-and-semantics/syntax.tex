...
\olsection{Syntax of \textbf{PL}}


We denote the language of propositional logic by $\Lang L_{PL}$.
...
\begin{enumerate}
\item !!^a{denumerable} set~$\PVar$ of !!{propositional variable}s $\Obj P$, $\Obj P_0$,
  $\Obj P_1$, \dots, $\Obj Q$, $\Obj Q_0$, \dots
...
\paragraph{Definition of \usetoken{P}{formula}}

\begin{defn}[Formula]
\ollabel{defn:formulas}
The set~$\Lang{L_{PL}}$ of \emph{!!{formula}s} of propositional logic
is defined inductively as follows:
\begin{enumerate}
\tagitem{prvFalse}{$\lfalse$ is !!a{formula}.}{}

\tagitem{prvTrue}{$\ltrue$ is !!a{formula}.}{}

\item Every !!{propositional variable}~$\Obj p_i$ is a
  !!{formula}.
...
\ollabel{defn:atomic-formulas}
 !!^{propositional variable}s \iftag{prvFalse}{\iftag{prvTrue}{,}{ and} the
  propositional constant~$\lfalse$}{}\iftag{prvTrue}{\iftag{prvFalse}{
    and}{} the propositional constant~$\ltrue$}{} are called \emph{atomic} formulas.
