...
\olsection{Proof of the Interpolation Theorem}


\begin{proof}[Proof of Theorem \olref{thm:interpol}]
...
\item\ollabel{part-b} $\Gamma_{n+1}$ and $\Delta_n$ are inseparable in
    $\Lang{L}'_0$.
\end{enumerate}
The basis for \olref{part-a} is given by Lemma \olref[sep]{lem:sep1}. For
...
\item It remains to consider the case where $!A_0$ is existential, so
  ...
  $\Delta_0$ are inseparable, so that by Lemma \olref{lem:sep2} also
...
existential, by \olref{lem:sep2}); if $\Delta_{n+1} = \Delta_n$
...
by \olref{lem:sep2}); and if  $\Gamma_{n+2} = \Gamma_{n+1}$ then
...
\olref{thm:consis}. Similarly, $\Delta^*$ has a model $\Struct{M}'_2$
