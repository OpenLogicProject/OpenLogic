...
\[ \Gamma \Sequent \Delta \]
where $\Gamma$ and $\Delta$ are finite (possibly empty) sets of !p{formula} of the language $\Lang L$. The !p{formula} in $\Gamma$ are the \emph{antecedent formulae}, while the formulae in $\Delta$ are the \emph{succedent formulae}. 
...
For the following, let $\Gamma, \Delta, \Pi, \Lambda$ represent finite sets of !p{formula}, and let $!A, !B, !C, !D, \ldots$ represent individual !p{formula}.
