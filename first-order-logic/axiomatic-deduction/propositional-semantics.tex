...
\begin{proof} Apply Theorem \olref{thm:rec}. \end{proof}
...
\begin{thm} [Local Determination] \ollabel{thm:LocalDetermination}  Suppose
...
\begin{prob}\ollabel{prob:semanticalfacts} The following can all be proved
...
is also satisfiable; \item \ollabel{def:Monotony} Monotony: if $\Gamma
\subseteq \Delta$ and $\Gamma \models !A$ then also $\Delta \models !A$;
\item \ollabel{def:Cut} Cut: if $\Gamma \models !A$ and $\Delta \cup \{
!A\} \models !B$ then $\Gamma \cup \Delta \models !B$; \item
\ollabel{def:DecudtionTheorem} Deduction theorem: $\Gamma \models !A
