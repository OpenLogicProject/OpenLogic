...
\begin{enumerate} 
\item \ollabel{prop:easyproofs:trans} \emph{Transitivity}: $\{!A \lif
  !B, !B \lif !C\} \Proves !A \lif !C$.
\item \ollabel{prop:easyproofs:refl} $\Proves !A \lif !A$.
\end{enumerate} 
\end{prop}

\begin{proof} 
For part \olref{prop:easyproofs:trans}, the
...
\begin{prop}
\ollabel{prop:contain} 
For any set $\Gamma$ of !!{formula}s:
\begin{enumerate} 
\item If $!A$ is an axiom then $\Gamma \Proves !A$; 
\item \ollabel{prop:contain:reflex} If $!A$ is in $\Gamma$ then
  $\Gamma \Proves !A$;
\item \ollabel{prop:contain:monotony} \emph{Monotony}: if $\Gamma
...
\begin{proof} 
Part \olref{derivfacts:a} follows from \olref{prop:easyproofs},
part~\olref{prop:easyproofs:trans} by two applications of the
Deduction Theorem. For part \olref{derivfacts:b}:
...
\noindent For part~\olref{derivfacts:c} we have $\{\lnot !A, \lnot !B
\} \Proves \lnot !A$ by \olref{prop:contain}, part
\olref{prop:contain:reflex}, and $\{ !A, \lnot!A\} \Proves !B$ follows
by \olref{derivfacts:b}. Part \olref{derivfacts:d}: since
$\{\lnot\lnot !A, \lnot !A \} \Proves \lnot \lnot\lnot !A$ by \emph{ex
  falso quodlibet}, apply \olref{derivfacts:b}. Now for part
\olref{derivfacts:e}, $\Gamma \Proves \lnot\lnot !A \supset !A$ by the
...
\olref{prop:contain}\olref{prop:contain:monotony}. 
\end{proof}
...
\begin{proof} 
For part \olref{prop:consistent:0}, we have $\lnot\lnot\lnot!B \Proves
\lnot !B$ by double negation, whence $!B \Proves \lnot \lnot !B$ by
Prop. \olref{prop:derivfacts}, part \olref{derivfacts:b}. For part
\olref{prop:consistent:1} we have:
...
\smallskip\noindent Part \olref{prop:consistent:2} follows
because:
...
\olref{prop:consistent:3}: from the hypotheses by the
deduction theorem, $\Gamma \Proves !A \lif !B$ and $\Gamma \Proves !A \lif
\lnot !B$; from part \olref{prop:consistent:2} and monotony,
