...
\begin{defn}[Tautological Instance] A !!{formula} $!A$ of predicate logic
...
\begin{thm}\ollabel{thm:deduction-thm} \emph{Deduction Theorem}: $\Gamma
...
\begin{prop}\ollabel{prop:derivfacts} The following are all !!{derivable}:
\begin{enumerate} 
\item $\Proves (!A \lif !B) \lif ((!B \lif !C) \lif (!A
\lif !C)$; \ollabel{derivfacts:a} 
...
\Proves !B$; \ollabel{derivfacts:c} 
\item \emph{Double Negation}: $\{
\lnot\lnot!A\} \Proves !A$;\ollabel{derivfacts:d} 
\item if $\Gamma \Proves
\lnot\lnot!A$ then $\Gamma \Proves !A$;\ollabel{derivfacts:e}
\end{enumerate} \end{prop}


\begin{proof} Part \olref[fol][axd][rul]{derivfacts:a} follows from Prop.
\olref[fol][axd][rul]{prop:easyproofs}, part
\olref[fol][axd][rul]{prop:easyproofs:trans} by two applications of the
Deduction Theorem. For part \olref[fol][axd][rul]{derivfacts:b}:
...
\noindent For part \olref[fol][axd][rul]{derivfacts:c} we have $\{\lnot !A,
\lnot !B \} \Proves \lnot !A$ by Prop \olref[fol][axd][rul]{prop:contain},
part \olref[fol][axd][rul]{prop:contain:reflex}, and $\{ !A, \lnot!A\}
\Proves !B$ follows by \olref[fol][axd][rul]{derivfacts:b}. Part
\olref[fol][axd][rul]{derivfacts:d}: since $\{\lnot\lnot !A, \lnot !A \}
\Proves \lnot \lnot\lnot !A$ by \emph{ex falso quodlibet}, apply
\olref[fol][axd][rul]{derivfacts:b}. Now for part
\olref[fol][axd][rul]{derivfacts:e}, $\Gamma \Proves \lnot\lnot !A \supset
...
\olref[fol][axd][rul]{prop:contain}, part
\olref[fol][axd][rul]{prop:contain:monotony}. 
\end{proof}

\begin{def}\ollabel{def:trivial} $!A \lif !B, !A \Proves !B$. 
\end{def}
\begin{proof} Clearly $!A \lif !B \Proves !A\lif !B$. Apply the Deduction
Theorem. 
\end{proof}

\begin{def}\ollabel{def:notphi} $!A \lif \lnot !A \Proves \lnot !A$.
\end{def}
...
\begin{prop} \ollabel{prop:consistent} The following hold:
\begin{enumerate} 
\item $!B \Proves \lnot \lnot !B$\ollabel{prop:consistent:0} 
\item $!A \lif \lnot !B, !B \Proves \lnot!A$ \ollabel{prop:consistent:1} 
\item $!A\lif !B,!A\lif\lnot !B \Proves \lnot!A$. \ollabel{prop:consistent:2} 
\item if $\Gamma \cup \{ !A \}\Proves !B$ and $\Gamma \cup \{ !A\} 
\Proves \lnot !B$ then $\Gamma \Proves \lnot!A$. \ollabel{prop:consistent:3} 
\end{enumerate} 
\end{prop}

\begin{proof} For part \olref[fol][axd][rul]{prop:consistent:0}, we have
$\lnot\lnot\lnot!B \Proves \lnot !B$ by double negation, whence $!B \Proves
\lnot \lnot !B$ by Prop. \olref[fol][axd][rul]{prop:derivfacts}, part
\olref[fol][axd][rul]{derivfacts:b}. For part
\olref[fol][axd][rul]{prop:consistent:1} we have:
...
\smallskip\noindent Part \olref[fol][axd][rul]{prop:consistent:2} follows
because:
...
\olref[fol][axd][rul]{prop:consistent:3}: from the hypotheses by the
deduction theorem, $\Gamma \Proves !A \lif !B$ and $\Gamma \Proves !A \lif
\lnot !B$; from part \olref[fol][axd][rul]{prop:consistent:2} and monotony,
