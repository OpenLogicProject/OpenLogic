...
free for~$x$ in $!A$, then $\Subst{!A}{t}{x}$ is the result of substituting
$t$ for all free occurrences of $x$ in~$!A$.
\begin{enumerate}
...
and $t \ident y$, then $\Subst{!A}{t}{y}$ would be $\lexists[y][y <
  y]$, and this case where the free variable $y$ is ``captured'' by
the quantifier $\lexists[y]$ upon substitution, is undesirable.  For
instance, we would like it to be the case that whenever
$\lforall[x][!B]$ holts, so does $\Subst{!B}{t}{x}$. But if we take
$!B$ to be $\lforall[x][\lexists[y][x < y]]$ we have a sentence that
is true about, e.g., the natural numbers: for every number~$x$ there
is a number~$y$ greater than it. If we allowed $y$ as a possible
substitution for~$x$, we would end up with $\lexists[y][y < y]$, which
is false. We prevent this by requiring that none of the free variables
in~$t$ would end up being bound by a quantifier in~$!A$.
\end{explain}
