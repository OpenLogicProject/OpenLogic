...
\begin{enumerate}
\tagitem{prvFalse}{Never $\Sat{M}{\lfalse}[s]$.}{}
\tagitem{prvTrue}{\item Always $\Sat{M}{\ltrue}[s]$.}{}
...
\tagitem{prvNot}{$\Sat{M}{\lnot !A}[s]$ iff $\Sat/{M}{!A}[s]$.}{}
\tagitem{prvAnd}{$\Sat{M}{!A \land !B}[s]$ iff $\Sat{M}{!A}[s]$ and
    $\Sat{M}{!B}[s]$.}{}
\tagitem{prvOr}{$\Sat{M}{!A \lor !B}[s]$ iff $\Sat{M}{!A}[s]$ or
  $\Sat{M}{!B}[s]$ (or both).}{}
\tagitem{prvIf}{$\Sat{M}{!A \lif !B}[s]$ iff $\Sat/{M}{!A}[s]$ or
  $\Sat{M}{!B}[s]$ (or both).}{}
\tagitem{prvEx}{$\Sat{M}{\lexists[x][!A]}[s]$ iff there is an $x$-variant $s'$
  of $s$ so that $\Sat{M}{!A}[s']$.}{}
\tagitem{prvAll}{$\Sat{M}{\lforall[x][!A]}[s]$ iff for every $x$-variant $s'$ of
  $s$, $\Sat{M}{!A}[s']$.}{}
\end{enumerate}
\end{defn}
...
\iftag{prvTrue}{When $!A$ is $\ltrue$, both $\Sat{M}{!A}[s]$ and
  $\Sat{M}{!A}[s']$.}{}
\iftag{prvFalse}{\iftag{prvTrue}{Similarly, w}{W}hen $!A$ is
  $\lfalse$, both $\Sat!{M}{!A}[s]$ and $\Sat!{M}{!A}[s']$.}{}
...
\begin{enumerate}
\tagitem{defNot}{}{$!A$ is $\lnot !B$:
  \iftag{probNot}{Exercise}{If $\Sat{M}{\lnot !B}[s]$,
  then $\Sat!{M}{!B}[s]$, so by the induction hypothesis,
  $\Sat!{M}{!B}[s']$, hence $\Sat{M}{\lnot !B}[s']$}.}

\tagitem{defAnd}{}{$!A$ is $!B \land !C$: 
  \iftag{probAnd}{Exercise}{If $\Sat{M}{!B \land !C}[s]$, then
    $\Sat{M}{!B}[s]$ and $\Sat{M}{!C}[s]$, so by induction hypothesis,
    $\Sat{M}{!B}[s']$ and $\Sat{M}{!C}[s']$. Hence, $\Sat{M}{!B \land
      !C}[s']$}.}

\tagitem{defOr}{}{$!A$ is $!B \lor !C$:
  \iftag{probOr}{Exercise}{$\Sat{M}{!B \lor !C}[s]$, then
    $\Sat{M}{!B}[s]$ or $\Sat{M}{!C}[s]$. By the induction hypothesis,
    $\Sat{M}{!B}[s']$ or $\Sat{M}{!C}[s']$, so $\Sat{M}{!B \lor
      !C}[s']$}.}

\tagitem{defIf}{}{$!A$ is $!B \lif !C$:
  \iftag{probIf}{Exercise}{If $\Sat{M}{!B \lif !C}[s]$,
  then $\Sat{M}{!B}[s]$ or $\Sat{M}{!C}[s]$. By the induction
  hypothesis, $\Sat{M}{!B}[s']$ or $\Sat{M}{!C}[s']$, so $\Sat{M}{!B
    \lif !C}[s']$}.}

\tagitem{defEx}{}{$!A$ is $\lexists[x][!B]$:
  \iftag{probEx}{Exercise}{If $\Sat{M}{\lexists[x][!B]}[s]$, there is
    an $x$-variant $\bar{s}$ of $s$ so that
    $\Sat{M}{!B}[\bar{s}]$. Let $\bar{s}'$ denote the $x$-variant of
    $s'$ that assigns the same thing to $x$ as does $\bar{s}$: then by
    the induction hypothesis, $\Sat{M}{!B}[\bar{s}']$. Hence, there is
    an $x$-variant of $s'$ that satisfies $!B$, so
    $\Sat{M}{\lexists[x][!B]}[s']$}.}

\tagitem{defAll}{}{$!A$ is $\lforall[x][!B]$:
  \iftag{probAll}{Exercise}{If $\Sat{M}{\lforall[x][!B]}[s]$, then for
    every $x$-variant $\bar{s}$ of $s$, $\Sat{M}{!B}[\bar{s}]$. Hence,
    if $\bar{s}'$ is the $x$-variant of $s'$ that assigns the same
    thing to $x$ as does $\bar{s}$, then we have
    $\Sat{M}{!B}[\bar{s}']$. Hence, every $x$-variant of $s'$
    satisfies $!B$, so $\Sat{M}{\lforall[x][!B]}[s']$}.}
\end{enumerate}
...
\begin<probNot,probOr,probAnd,probIf,probEx,probAll>{prob}
Complete the proof of \olref{prop:satindep}.
\end{prob}
