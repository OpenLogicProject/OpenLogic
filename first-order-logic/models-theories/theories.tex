...
\begin{ex}
The theory of linear orders in the language~$\Lang L_\le$ is
...
& \lforall[x][\lforall[y][(x \le y \lor y \le x)]] \quad \}
\end{align*}
It completely captures the intended !!{structure}s: every linear order
is a model of this axiom system, and vice versa, if $R$ is a linear
order on a set $X$, then the structure $\Struct M$ with $\Domain M =
X$ and $\Assign{\le}{M} = R$ is a model of this theory.
\end{ex}
...
\]
and this !!{sentence} is not true in any !!{structure}.
\end{ex}
