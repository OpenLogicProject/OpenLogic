...
\begin{enumerate}
\item \indcase{!A}{R(t_1, \dots, t_n)}{$\tuple{t_1, \dots, t_n} \in
  \Assign{R}{M(\Gamma^*)}$ by the definition of satisfaction, and by
  the construction of $\Struct M(\Gamma^*)$, $R(t_1, \dots, t_n) \in
  \Gamma^*$.}
...
\indcase{!A}{\lexists[x][!B(x)]}{since $\Sat{M(\Gamma^*)}{\indfrm}$, for
  ...
  value $s(x)$ is some !!{constant}~$c \in \Domain{M(\Gamma^*)}$. Thus,
    ...
    $!B(c) \in \Gamma^*$.  By \olref[seq][prv]{thm:provability-quantifiers}
