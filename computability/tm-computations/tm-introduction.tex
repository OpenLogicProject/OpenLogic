...
alphabets can contain any number of different symbls, but we will
...
q_j, \sigma', D\rangle$. Whenever the mecahnism is in state $q_i$ and
reads symbol $\sigma$, it replaces the symbol on the current square
with $\sigma'$, the head moves left, right, or stays put acording to
whether $D$ is $L$, $R$, or $N$, and the mechanism goes into
state~$q_j$.  When the mecahnism enters state~$h$ we say it
