...
!!^{proof}s in \Log{N1c} and~\Log{N1i} are !!{proof}s of !!{formula}s~$!A$ from
...
In \Log{N1c} and~\Log{N1i} we can actually avoid it in this case quite easily: we
...
\begin{defn}
If $\delta_1$ is a \Log{N1c}-!!{proof} (or \Log{N1i}-!!{proof})
of~$!A$ from open assumption~$!B^x$, and $\delta_2$ is a
\Log{N1c}-!!{proof} (\Log{N1i}-!!{proof}) of~$!B$, then
$\Subst{\delta_1}{\delta_2}{!B^x}$ is the result of replacing every
undischarged occurrence of~$!B^x$ in~$\delta_1$ by~$\delta_2$.
\end{defn}
...
A corresponding definition applies to \Log{N2c} and~\Log{N2i}.

\begin{defn}
If $\delta_1$ is a \Log{N2c}-!!{proof} (or \Log{N2i}-!!{proof})
of~$x:!B, \Gamma_1 \Sequent !A$, and $\delta_2$ is a
\Log{N1c}-!!{proof} (\Log{N1i}-!!{proof}) of~$\Gamma_2 \Sequent !B$,
then $\Subst{\delta_1}{\delta_2}{x:!B}$ is the result of replacing
every axiom $x:!B \Sequent !B$ in~$\delta_1$ by~$\delta_2$, and adding
$\Gamma_2$ to the context of every sequent below such axioms.
\end{defn}
