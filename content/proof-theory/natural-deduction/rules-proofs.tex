...
Consider the two rules in \Log{N1c} and~\Log{N1i} that mention $!A
\lif !B$.
\[
\bottomAlignProof
...
are the \emph{premises}. The !!{formula} on the left is the
\emph{principal} !!{formula}~$!A \lif !B$. All !!{elimination} rules
have one such premise, always on the far left. It is called the
\emph{major} premise of the inference. If there are others, such as an
this case, they are called \emph{minor} premises. The premise(s) of
!!{introduction} rules are also called major premise(s). The
!!{formula} below the line is the \emph{conclusion}.
...
correct, even though the first \Intro{\lif} does not discharge any
assumptions:
\[
...
!!^{proof}s in \Log{N1c} and~\Log{N1i} are finite trees of sequents
that are inductively generated from axioms using the rules of
inference. Each leaf is an assumption, possibly labelled with a
discharge label, and every other !!{formula} follows from the
!!{formula}s immediately above it by one of the inference rules of the
system. If an occurence of a !!{formula} as an assumption in the tree
above an inference has not been discharged by an inference above it,
it is called \emph{open} (at the inference).

\begin{defn}\ollabel{defn:proof}
\emph{!!^{proof}s} in \Log{N1c} and~\Log{N1i} are finite trees of
