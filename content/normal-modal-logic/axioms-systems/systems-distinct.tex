...
\begin{figure}[htpb]
  ...
  \begin{tikzpicture}[modal]
    \node[world] (w1) [label=left:$w$,
    label={below:$\mSat{}{\Box p}$\\ $\mSat/{}{\Box\Box p}$}]{}; 
    \node[world] (w2) [label=right:$w'$,
      label=below:$\mSat/{}{\Box p}$,right=of w1]{$p$}; 
...
\begin{proof}
  ...
  at which some instance of~\Ax{4} fails, and similarly for~\Ax{5}.
  We use the same model for both claims. Consider the symmetric,
  reflexive model in \olref{fig:KTBnot45}.  Then $\mSat/{M}{\Box p \to
    \Box \Box p}[w_1]$, so \Ax{4} fails at $w_1$. Similarly,
  $\mSat/{M}{\Diamond \lnot p \lif \Box \Diamond \lnot p}[w_2]$, so
  the instance of~\Ax{5} with $!A = \lnot p$ fails at~$w_2$.
\end{proof}

\begin{figure}[htpb]
  \centering
  \begin{tikzpicture}[modal]
    \node[world] (w1) [label={left:$w_1$}, label={below: $\mSat{}{\Box p}$\\
        $\mSat/{}{\Box\Box p}$\\
        $\mSat/{}{\Diamond\lnot p}$}] {$p$};
    \draw[reflexive above] (w1) to (w1);
    \node[world] (w2) [label={below: $w_2$\\
        $\mSat{}{\Diamond\lnot p}$\\
        $\mSat/{}{\Box\Diamond\lnot p}$}, right=of w1] {$p$} ;
    \draw[reflexive above] (w2) to (w2);
    \node[world] (w3) at (4, 0) [label={right:$w_3$},right=of w2] {};
    \draw[reflexive above] (w2) to (w2);
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[node distance=2cm, auto, thick]
    \node[world] (w2) [label=north west:$w_2$]{$p$} ;
    \draw[reflexive left] (w2) to (w2);
    \node[world] (w1) [label=right:$w_1$,label=below:{$\mSat{}{\Box p}, \mSat/{}{\Box\Box p}$},below right=of w2]{};
    \node[world] (w3) [label=north east:$w_3$, above right=of w1]{$p$} ;
    \draw[reflexive right] (w3) to (w3);
    \node[world] (w4) [label=right:$w_4$,above right=of w2]{};
