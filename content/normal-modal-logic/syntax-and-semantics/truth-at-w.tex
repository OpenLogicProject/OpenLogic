...
\begin{defn}\ollabel{defn:mmodels}
  ...
  \tagitem{prvIf}{\indcase{!A}{(!B \land !C)}{$\mSat{M}{\indfrm}[w]$ iff
    $\mSat/{M}{!B}[w]$ or $\mSat{M}{!C}[w]$}.}{}
  \tagitem{prvIff}{\indcase{!A}{(!B \land !C)}{$\mSat{M}{\indfrm}[w]$ iff
...
\begin<prvDiamond>{prop}\ollabel{prop:dual}
  \begin{enumerate}
...
\begin{probtag}{probDiamond}
  Complete the proof of \olref[mod][syn][trw]{prop:dual}.
\end{probtag}
