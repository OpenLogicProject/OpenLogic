...
are. We codify this in in the following principle.
\end{explain}
...
can also be written as $\{3, 2, 1\}$ or even as $\{1, 2, 1, 2\}$.
These are all the same set, by extensionality. For every !!{element}
of $\{1, 2, 3\}$ is also !!a{element} of $\{3, 2, 1\}$, and vice
versa.
\end{ex} 
...
are sets without members, then $A = B$.
\end{prob}
