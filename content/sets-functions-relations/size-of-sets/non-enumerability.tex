...
function $f\colon \Int^+ \to X$ can be surjective.  The best way of
doing this is Cantor's \emph{diagonal method}.  Given a list of
!!{element}s of $X$, say, $x_1$, $x_2$, \dots, we construct another
element of~$X$ which, by its construction, cannot possibly be on that
list.
...
element of the $i$-th sequence in this list $s_i(j)$.
