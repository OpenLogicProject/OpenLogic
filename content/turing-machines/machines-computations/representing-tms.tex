...
are an even number of $\TMstroke$s (strokes) on the tape (under the assumption that all $\TMstroke$s come before the first $\TMblank$ on the tape).
\[
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
                    semithick]
  \tikzstyle{every state}=[fill=none,draw=black,text=black]
...
\begin{ex}
  ...
  The state diagram of the resulting Turing machine is depicted in
  \olref{fig:doubler}.
  \begin{figure}
\[
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
