...
\item The last inference is \RightR{\lforall}: Then there is a
  ...
  $\Struct{M'}{!C}$ since $a$ does not occur in~$\Gamma$, and for any
  $!C \in \Delta$, $\Struct/{M'}{!C}$. But the premise is valid, so
...
\quad$.  By \olref{thm:sequent-soundness}, $\Gamma_0 \Sequent \quad$
