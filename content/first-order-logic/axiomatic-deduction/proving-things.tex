...
given $!A$ and $!A \lif !B$ allows us to justify~$!B$. The obvious
...
\begin{ex}
Let's try to find !!a{derivation} of $!D \lif !D$.  In
\olref[prp]{ax:lif1}, we have an axiom of the form~$!A \lif !B$ to
which we could apply~MP. To be useful, of course, the $!B$ which MP
would yield in this case would have to be~$!D \lif !D$. That means
$!A$ would also have to be $!D$, i.e., we might look at this instance
of \olref[prp]{ax:lif1}:
\[
!D \lif (!D \lif !D)
\]
...
\begin{prop}
  \ollabel{prop:chain} If $\Gamma \Proves !A \lif !B$ an $\Gamma
