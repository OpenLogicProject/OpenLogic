...
\ollabel{thm:deduction-thm-q} If $\Gamma \cup \{!A\} \Proves !B$ and
...
rule is QR$_1$, we know that $!B \ident !C \lif \lforall[x][!D(x)]$ and
...
We leave the case where $!B$ is justified by QR$_2$ as an exercise.
\end{proof}
