...
cannot define satisfaction of $\lforall[x][!B(x)]$ by ``for all $a \in
\Domain{M}$, $\Sat{M}{!B(a)}$.''}{}\iftag{prvEx}{ We cannot define
...
!!{structure}s that requires it.  In the standard language, the set of
!!{constant}s is !!{denumerable}, so if $\Domain{M}$ is not
