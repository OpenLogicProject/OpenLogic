...
variable assignment to each variable.\iftag{fnTerms}{But we want to use terms built
...
assignment specifies it\iftag{fnTerms}{; for more complex terms it is computed
...
\begin{ex}
Let \iftag{fnTerms}{$\Lang = \{a, b, f, R\}$}{$\Lang = \{a, b, R\}$} 
 $a$ and $b$ are !!{constant}s, \iftag{fnTerms}{$f$
...
\item $\Assign{a}{M} = 1$
\item $\Assign{b}{M} = 2$
\tagitem{fnTerms}{ $\Assign{f}{M}(x, y) = x+y$ if $x+y \le 3$ and $= 3$ otherwise.}{}
...
\iftag{fnTerms}{%
  we have $\Sat{M}{R(b,f(a,b))}[s]$ since $\tuple{\Value{b}{M},
  \Value{f(a,b)}{M}} = \tuple{2, 3} \in \Assign{R}{M}$, but
$\Sat/{M}{R(x, f(a,b))}[s]$ since $\tuple{1, 3} \notin \Assign{R}{M}[s]$.
}{%
  we have $\Sat{M}{\Atom{R}{x,b}}[s]$ since $\tuple{\Value{x}{M}[s],
  \Value{b}{M}[s]} = \tuple{1,2} \in \Assign{R}{M}$, but $\Sat/{M}{\Atom{R}{b,x}}[s]$ 
  since $\tuple{\Value{b}{M}[s],
  \Value{x}{M}[s]} = \tuple{2,1} \notin \Assign{R}{M}$.
}
...
\iftag{prvAll}{To determine if a universally quantified
  ...
  ($\Sat{M}{R(a,x)}[s_1]$ and $\Sat/{M}{R(x,a)}[s_j]$ for $j = 2$,
  $3$, and~$4$).  But,
  \[
