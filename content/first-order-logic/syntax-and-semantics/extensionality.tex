...
\begin{prop}[Extensionality]
  ...
  $\Assign{c}{M_1} = \Assign{c}{M_2}$, \iftag{fnTerms}{}{and }$\Assign{R}{M_1}=\Assign{R}{M_2}$
  \iftag{fnTerms}{, and $\Assign{f}{M_1} = \Assign{f}{M_2}$}{} for every !!{constant}~$c$,
  \iftag{fnTerms}{}{and }relation symbol~$R$\iftag{fnTerms}{, and !!{function} $f$}{} occurring in~$!A$, then
  $\Sat{M_1}{!A}[s]$ iff $\Sat{M_2}{!A}[s]$.
\end{prop}

\begin{proof}
  First prove \iftag{fnTerms}{(by induction on~$t$)}{} that for every term,
...
\begin{proof}
\iftag{fnTerms}{By induction on~$t$.}{}
\begin{enumerate}
...
\tagitem{fnTerms}{If $t \ident \Atom{f}{t_1,\dots,t_n}$ then we have:
\begin{multline*}
