...
\begin{enumerate}
  \item[\rule{VAR}] \ollabel{form:var} If $x$ is a variable, then $x$ is a
    term called \emph{Variable}.
  \item[\rule{ABS}] \ollabel{form:abs} If $x$ is a variable and $M$ is a term, then $(\lambd[x][M])$ is
    a term called an \emph{abstraction}. The $x$ in
    $\lambd[x]$ is called a \emph{parameter}, or \emph{binding variable}.
  \item[\rule{APP}] \ollabel{form:app} If both $M$ and $N$ are terms, then
