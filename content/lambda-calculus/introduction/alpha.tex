...
\emph{compatibility}. A relation $R$ on terms is said to be compatible
...
\begin{defn}[$\alpha$-conversion, $\aconv$]
  $\alpha$-conversion ($\aconv$) is the smallest reflexitive, transitive relation on terms containing $\aconvone$.
\end{defn}
...
\begin{defn}[$\alpha$-conversion, $\aconv$]
  $\alpha$-conversion ($\aconv$) is inductively defined as follows:
  \begin{enumerate}
...
\begin{proof}
  ...
  \begin{enumerate}
  \item[\rule{ABS}] Then $M$ is of the form $\lambd[x][N]$, then we:
    \begin{enumerate}
    ...
    \item Moreover, if there's another $N''$ and $R''$ satisfying such
      ...
      \end{align*}
    \end{enumerate}
  \item[\rule{VAR},\rule{APP}] The other two cases are left as
    exercises.
  \end{enumerate}
\end{proof}
