...
\begin{tikzpicture}[modal]
  ...
  \path (1.3,.6) node[world] {$w_2$};
  \path (-.4,-1.4) node[world] {$w_3$};
  \path (0,1.5) node[world] {$w_1$};
  \begin{scope}
    \draw[clip] plot [smooth,tension=1.5] coordinates { (5,3) (2.3,0) (5,-3)};
    \filldraw[fill=lightgray] (0,0) circle (3.6);
  \end{scope}
  \path (5,0) node[left] {$p$};
  \path (2.9,.6) node[world] {$w_5$};
  \path (2.9,-.6) node[world] {$w_6$};
  \path (-2,-2) node[world] {$w_4$};
  \path (4.2,1.5) node[world] {$w_7$};
\end{tikzpicture}
