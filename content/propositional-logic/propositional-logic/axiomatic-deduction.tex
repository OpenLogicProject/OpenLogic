...
\begin{proof}
We know from \olref{prop:contain} that $\Thms{\Gamma}$ has the desired
...
\begin{proof}
Part \olref{derivfacts:a} follows from \olref{prop:easyproofs}, part
...
For part \olref{derivfacts:c} we have $\{\lnot !A,
...
\end{derivation}
``Monotony'' refers to \olref{prop:contain}\olref{prop:contain:monotony}.
\end{proof}
...
The following hold:
\begin{enumerate}
\item \ollabel{prop:consistent:0} $!B \Proves \lnot \lnot !B$
\item \ollabel{prop:consistent:1} $!A \lif \lnot !B, !B \Proves
  \lnot!A$
\item \ollabel{prop:consistent:2} $!A\lif !B,!A\lif\lnot !B \Proves
  \lnot!A$.
\item \ollabel{prop:consistent:3} If $\Gamma \cup \{ !A \}\Proves !B$
...
\lnot !B$ by \olref{prop:derivfacts}, part
...
\begin{derivation}
  1. & $!A \lif !B, !A \Proves !B $ & \olref{lem:trivial} \\
  ...
  lemma \olref{lem:notphi} \\
  6. & $!A \lif !B, !A \lif \lnot !B
  \Proves \lnot !A$ & 4, 5, Cut.
\end{derivation}

Finally, for part \olref{prop:consistent:3}: from the hypotheses by
the deduction theorem, $\Gamma \Proves !A \lif !B$ and $\Gamma \Proves
!A \lif \lnot !B$; from part \olref{prop:consistent:2} and monotony,
...
\begin{prob}
Prove \olref[prp][prp][axd]{prop:incons}
\end{prob}
...
\olref{prop:consistent}\olref{prop:consistent:3}, $\Gamma
...
\begin{proof}
From \olref{prop:prov-incons} we have that if $\Gamma \cup \{\lnot !A\}$ is
