...
\item There are $z_1, z_2 < x$ such that $\fn{Term}(z_1)$,
  $\fn{Term}(z_2)$, and
\[
x = \Gn{\eq(} \concat z_1 \concat \Gn{,} \concat z_2 \concat{)}.
\]
...
The relation $\fn{FreeOcc}(x, z, i)$, which holds if the $i$-th
symbols of the formula with G\"odel number~$x$ is a free occurrence of
...
\[
\forall i<\len{x}\forall z<x(\exists j<z\, z=\Gn{\Obj v_j}
