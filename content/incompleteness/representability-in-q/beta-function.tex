...
\begin{enumerate}
\item\ollabel{rel-prime} $x_0,\dotsc,x_n$ are relatively prime.
...
\end{align*}
This is the function we want. Given $a_0,\dotsc,a_n$ as above, let
\[
j = \max(n,a_0+1,\dotsc,a_n+1),
\]
and let $d_1 = \lcm(1,\dotsc,j)$. By \olref{rel-prime} above,
we know that $1+d_1$, $1+2 d_1$, \dots, $1+(n+1) d_1$ are relatively
prime, and by~\olref{less} that all are greater than $a_0,\dotsc,a_n$.
