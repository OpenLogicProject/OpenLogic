...
incompleteness theorem, we'll take $!B(x)$ to be ``$x$ is unprovable
in~$\Th{T}$.'' Then $!E(x)$ would be ``yields !!a{sentence} unprovable
...
$\Obj{diag}$ were a function symbol in $\Th{T}$ repqresenting the
...
$\fn{diag}(\Gn{!E(x)}) = \Gn{!A}$, $\Th{Q}$ can prove
\begin{align}
...
informally, using just logic and facts provable in~$\Th{Q}$.
