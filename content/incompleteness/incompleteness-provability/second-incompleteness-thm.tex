...
& \OProv(\gn{!G}) \lif
  ...
  & \qquad\text{from \olref{G2-3} by condition P2} \notag\\
& \OProv(\gn{!G}) \lif (\OProv(\gn{\OProv(\gn{!G})}) \lif \OProv(\gn{\lfalse})) \ollabel{G2-5}\\
  & \qquad\text{from \olref{G2-4} by condition P2 and logic} \notag\\
& \OProv(\gn{!G}) \lif 
  \OProv(\gn{\OProv(\gn{!G})}) \ollabel{G2-6}\\
   & \qquad\text{by P3} \notag\\
& \OProv(\gn{!G}) \lif \OProv(\gn{\lfalse}) \ollabel{G2-7}\\
  & \qquad \text{from \olref{G2-5} and \olref{G2-6} by logic}\notag\\
& \OCon \lif \lnot \OProv(\gn{!G}) \ollabel{G2-8}\\
  & \qquad\text{contraposition of \olref{G2-7} and $\OCon \ident \lnot \OProv(\gn{\lfalse})$}\notag \\
& \OCon \lif !G \notag\\
  & \qquad\text{from \olref{G2-1} and \olref{G2-8} by logic}\notag
\end{align}
The use of logic in the above just elementary facts from propositional
logic, e.g., \olref{G2-3} uses $\Proves \lnot!A \liff (!A\lif
\lfalse)$ and \olref{G2-7} uses $!A \lif (!B \lif !C), !A \lif !B
\Proves !A \lif !C$. The use of condition~P2 in \olref{G2-4} and
\olref{G2-5} relies on instances of P2, $\OProv(\gn{!A \lif !B}) \lif
