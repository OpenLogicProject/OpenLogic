...
\begin{defn}
The theory of \emph{true arithmetic} is the sent of !!{sentence}s
...
!!{sentence}s entailed by some explicitly specified set of
sentences~$\Gamma_0$.  In that case, we call the !!{sentence}s
...
Another way to put this is to ask: Is $\Th{PA} = \Th{TA}$? For
...
\begin{defn}
A set $X$ is \emph{!!{decidable}} iff its characteristic function
$\Char{X} \colon X \to \{0, 1\}$, with
\[
\Char{X}(x) =
\begin{cases}
1 & \text{if $x \in X$} \\
...
is complete, and none of them are decidable. However, thisphenomenon
is not specific to these particular theories. In fact, \emph{any}
theory that satisfies certain conditions is subjec to the same
results. One of these conditions, which $\Th{Q}$ and $\Th{PA}$
satisfy, is that they are axiomatized by !!a{decidable} set of axioms.
...
!!{decidable} relations. By ``basic facts,'' we mean that the theories
in question count among its theorems !!{sentence}s which express what
the values of computable functions are for each of their arguments.
...
A theory is be ``strong enough'' for the incompleteness theorems to
apply if it !!{represents} all computable functions and all
!!{decidable} relations. $\Th{Q}$ and its extensions satisfy this
condition. $\Th{Q}$ is a very weak theory, so the condition is
realltively easily met. Certainly, any theory aiming to formalize
``all of mathematics'' must satisfy it, since it should at the very
least be able to capture the results of elementary computations.
